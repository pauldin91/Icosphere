...
void main() {    
    ...
    // gl_Position = modelview*displace((gl_in[0].gl_Position+gl_in[1].gl_Position)/2.0,normal); Tex_Coords_2=(vec2(1.0,0.0)+vec2(0.5,1.0))/2.0; aColor=internal; EmitVertex();
    // gl_Position = modelview*displace((gl_in[1].gl_Position+gl_in[2].gl_Position)/2.0,normal); Tex_Coords_2=(vec2(0.0,0.0)+vec2(1.0,0.0))/2.0; aColor=internal; EmitVertex();
    // gl_Position = modelview*displace((gl_in[2].gl_Position+gl_in[0].gl_Position)/2.0,normal); Tex_Coords_2=(vec2(0.0,0.0)+vec2(0.5,1.0))/2.0; aColor=internal; EmitVertex();
    // EndPrimitive();
/**/ 
/**/
// /**/    
    gl_Position = modelviewG * /*rotationMatrix(normal,3.14159265*time/180.0)  */  displace(h1,hnormal) ; Tex_Coords=(vec2(1.0,0.0)+vec2(0.5,1.0))/2.0; aColor=internal; EmitVertex();
    gl_Position = modelviewG * /*rotationMatrix(normal,3.14159265*time/180.0)  */  displace(h2,hnormal) ; Tex_Coords=(vec2(0.0,0.0)+vec2(1.0,0.0))/2.0; aColor=internal; EmitVertex();
    gl_Position = modelviewG * /*rotationMatrix(normal,3.14159265*time/180.0)  */  displace(h3,hnormal) ; Tex_Coords=(vec2(0.0,0.0)+vec2(0.5,1.0))/2.0; aColor=internal; EmitVertex();
    EndPrimitive();
}
